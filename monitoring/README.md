...
| 지표 | 최대 | 읽는 법 |
|---|---|---|
| `connections_pending` | **96** | c=100 의 거의 전부가 줄을 섰다 |
| `connections_active` | **20** (=상한) | 풀은 확실히 포화됐다 |
| `connections_acquire_seconds_max` | **7.28초** | 대기도 길었다 |
| `connections_timeout_total` | **0** | **그런데 아무도 안 죽었다** |

즉 **포화 ≠ 붕괴**다. 커넥션당 작업이 짧아(다운샘플 후 5행 INSERT) 7초 대기가 30초 한도에 못 미치고 큐가 빠진다. 이 네 줄은 **수치만 있었을 때는 구분되지 않던 상태**다 — "실패 0" 하나만 보면 풀이 한가한 것과 포화인데 버티는 것이 똑같아 보인다.
