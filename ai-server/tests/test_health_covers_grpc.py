...
def _reset_state():
    """모듈 전역이라 판마다 되돌린다 — 안 하면 판 순서가 결과를 바꾼다."""
    before = (grpc_server._serving, grpc_server._start_error)
    yield
    grpc_server._serving, grpc_server._start_error = before
...
def test_grpc_가_서빙_중이면_200():
    ...
    assert res.json()["grpc"]["serving"] is True


def test_grpc_가_안_떴으면_503_이고_사유가_실린다():
    """이 판이 #430 의 본체다 — 프로세스는 멀쩡한데 gRPC 만 죽은 상태."""
    grpc_server._serving = False
    grpc_server._start_error = "RuntimeError: INTERNAL_API_TOKEN 환경변수가 설정되지 않았습니다."

    res = _client().get("/health")

    assert res.status_code == 503, "컨테이너 healthcheck 가 이 코드를 보고 unhealthy 로 만든다"
    body = res.json()
    assert body["status"] == "degraded"
    assert body["grpc"]["serving"] is False
    assert "INTERNAL_API_TOKEN" in body["grpc"]["error"], "사유가 실려야 진단이 원인에 닿는다"


def test_토큰이_비면_기동이_실패하고_사유가_남는다():
    ...
    try:
        ...
        settings.INTERNAL_API_TOKEN = original

    serving, error = grpc_server.grpc_status()
    assert serving is False
    assert "INTERNAL_API_TOKEN" in (error or "")
