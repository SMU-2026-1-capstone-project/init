...
def detect_pose(req: PoseRequest):
    ...
    #
    # ⚠️ **세션에 보관값이 없으면(state.session_nonce is None) 여전히 통과한다.** 이건 남겨둔
    #    창이 아니라 **스스로 닫히는 창**이다 — 보관값이 없는 세션은 이 기능 배포 «전» 에
    #    시작된 것뿐이고, 세션 타임아웃(최대 45분)이 그것들을 걷어간다. 여기서 같이 막으면
    #    배포 순간 운동 중이던 사람이 끊기는데, 그 대가로 얻는 것이 45분짜리 창 하나뿐이다.
