...
PY
  ...
  🔴 팔 표기: A = 계측 OFF · B = 계측 ON · @<초> = GIL 스위치 간격.
     위 판은 **5팔 × 5×5 라틴 방격 + 버림 1 = 26판**이다 — A(기준) · B(구간·lease + 앵커) ·
     A@0.0005 · A@0.05(GIL **양방향**) · B@0.0005(계측×GIL **상호작용**, 2×2 를 닫는 칸).
     구간 비율·lease 는 **B 계열에서만** 걷힌다 — 계측 OFF 만으로 돌리면 판정선 셋 중 둘이 빈다.
  🔴 이 문자열의 정본은 docs/decisions/ai-receive-path-scaling.md §13 이다. **여기서 고치지 말 것** —
     고치면 격자가 두 벌이 된다(2026-08-23 에 실제로 한 번 그랬다).
