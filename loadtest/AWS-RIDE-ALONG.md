...
| **R10-a** ⭐ | **프레임 경로 계측 — 1대 판** — GIL 노브가 처리량을 움직이나 | 🟢 **무대 결정 완료 (2026-08-23 사용자 confirm)** — [`r10-loadgen-topology.md` §7](../docs/decisions/r10-loadgen-topology.md): **쪼갠다, R10-a 를 먼저 돌린다.** 후보 일곱이 반증되고 **하나가 안 갈렸다.** 1순위(서비스 경로 GIL)를 재려면 py-spy 로는 안 되고(§11 — 630 샘플 전부 파킹) **앱 계측**이 필요했는데, 그 계측이 [#384](https://github.com/Shadowfit/init/pull/384) 로 들어갔고 rig 이식은 [#396](https://github.com/Shadowfit/init/pull/396) 으로 닫혔다(리눅스 인터프리터·규모·**GIL 팔**). **이 판이 쓰는 판정선 셋** — `GIL_SWITCH_INTERVAL` 을 흔들어 처리량이 움직이면 **1순위 확정** · `wait` 이 크면 **GIL**(워커에 못 실린다) · `lease` 가 크면 **검출기 획득**(§10-2). 셋 다 **상대값이라 부하기 동거에도 산다**. 🔴 **`handler_concurrency` 는 이 판에서 안 읽는다** — 절대값이라 동거에서 다치고, **R10-b 로 미뤘다.** 🔴 **박스 조건은 안 낮아졌다** — 「346 RPS 에 9.5 vCPU」는 `c7i.4xlarge`(16 vCPU)의 관측이고, 쪼개서 아낀 것은 **부하기 박스와 그 배선**이지 대상 박스가 아니다. 로컬 물리 2코어로는 구조적으로 못 잰다(R6 이 확인한 벽). ⚠️ **계측은 켜도 된다** — 대가가 **A 팔 판 간 산포(2.4%) 이하**다([결과](results/frame-path-overhead-2026-08-23/README.md)). 단 **ON/OFF 앵커 판 한 장**은 끼울 것 | [`ai-receive-path-scaling.md` §10·§12](../docs/decisions/ai-receive-path-scaling.md) · [무대 결정](../docs/decisions/r10-loadgen-topology.md) · ⚠️ **소요 ~30분** · **`c7i.4xlarge` 1대**(부하기 동거) · 도커·MySQL·시딩 **불필요**(venv + `frames.json` + 부하기). rig 은 `results/frame-path-overhead-2026-08-23/` 의 `run_arms.py` — GIL 팔은 `--plan "A,A@0.001,…"` |
