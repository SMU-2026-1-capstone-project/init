...
| **R14** | **HTTP 읽기 경로 p99 — 판정선 대면** | 🟢 **rig 완성** — [`k6/read_p99_ec2.js`](k6/read_p99_ec2.js). 쓰기축(從 R13)과 **같은 형태**다: 가정 P1 배수 · `constant-arrival-rate` · 부하기 분리 · k6 v2.1.0 | **SLO 11번의 나머지 절반.** 읽기 절반은 [2026-08-23 로컬 판](results/http-read-p99-2026-08-23/README.md)이 처음 쟀지만 **판정선(p99 ≤ 1s)과 대면하지 못했다** — 그 판이 스스로 셋을 적어뒀다: ① 2코어 동거에 **부하기까지 동거** ② `slo-baseline` 이 「시간 임계는 이 환경에서 안 쓴다」고 못박음 ③ **16 VU 에 근거가 없다**. 이 rig 이 셋을 다 없앤다. 🔑 **쓰기축과 달리 rig 천장이 없다** — 읽기는 세션을 안 만들어 「계정 완결 왕복」 제약(R13 이 ×360 을 못 건 이유)에 안 걸린다. 🔴 **대신 데이터 소유권이 선결이다** — 시더가 `member_id` 를 1·5·12 로 하드코딩해서 새 계정엔 읽을 것이 없다. 러너가 **시드 단계를 먼저** 돌려 `K6_SIDS` 를 넘겨야 한다(로컬 판이 손으로 하던 자리). ⚠️ 그 시드가 이 항목의 유일한 «추가 비용» 이다 | [로컬 판](results/http-read-p99-2026-08-23/README.md) · [`slo-baseline.md` §4-2](../docs/decisions/slo-baseline.md) · 재고표 Spring 축 1번 |
...
- **2026-08-24: 자동 정지를 기본으로 켠다 (사용자 지시)**
  - `run_all.sh` 의 `AUTO_SHUTDOWN` 기본값을 `0 → 1`. 조건은 그대로 — **업로드 성공한 판에서만** 끈다
  - 🔴 **러너를 안 거치는 판**(rig 을 직접 부르는 단독 라운드)에는 이 노브가 안 붙는다.
    그런 판은 «끝나면 스스로 내려가는» 감시견을 user-data 나 별도 스크립트로 같이 심는다 —
    `DONE` 표식을 보고 `shutdown -h now`, 그리고 **상한 시각**을 둬서 판이 멈춰도 요금을 안 흘린다
  - ⚠️ `shutdown` 이 stop 인지 terminate 인지는 **인스턴스 속성**이다.
    `--instance-initiated-shutdown-behavior terminate` 로 띄우지 않으면 꺼져도 EBS 요금이 남는다
