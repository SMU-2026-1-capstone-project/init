...
| **R10** ⭐ | **프레임 경로 계측 본 판** — 「16 vCPU 중 9.5 만 쓰는」 원인 | 후보 일곱이 반증되고 **하나가 안 갈렸다.** 1순위(서비스 경로 GIL)를 재려면 py-spy 로는 안 되고(§11 — 630 샘플 전부 파킹) **앱 계측**이 필요했는데, 그 계측이 [#384](https://github.com/Shadowfit/init/pull/384) 로 들어갔다. **이제 남은 것은 판 하나다.** 걷는 값 넷이 후보를 가른다 — `wait` 이 크면 **GIL 확정**(워커에 못 실린다) · `lease` 가 크면 **검출기 획득**(§10-2) · `handler_concurrency` 가 **~9 에 눌리면** GIL 상한의 직접 증거 · `GIL_SWITCH_INTERVAL` 을 흔들어 처리량이 움직이면 1순위 확정. 🔴 **박스 조건이 까다롭다** — 「346 RPS 에 9.5 vCPU」는 `c7i.4xlarge`(16 vCPU)에서 난 관측이라 **그 박스에서만 재현된다.** 8 vCPU 로는 「9.5 of 16」이라는 질문 자체가 성립하지 않고, 로컬 물리 2코어로는 구조적으로 못 잰다(R6 이 확인한 벽). ⚠️ **계측을 켜도 된다** — 대가를 로컬에서 먼저 쟀고 **A 팔 판 간 산포(2.4%) 이하**로 묶였다([결과](results/frame-path-overhead-2026-08-23/README.md)). 단 그 라운드에도 **ON/OFF 앵커 판 한 장**은 끼울 것 | [`ai-receive-path-scaling.md` §10·§12](../docs/decisions/ai-receive-path-scaling.md) · ⚠️ **소요: 본 측정 ≈34분 · 인스턴스 1시간**(17판 = 버림 1 + 4팔 × 4 라틴 방격, 판당 90초 + 기동·정리 ~30초 — [격자](../docs/decisions/ai-receive-path-scaling.md) §13-7). 리드타임은 **리허설 재시도 횟수**가 정한다 · 도커·MySQL·시딩 **불필요**(venv + `frames.json` + 부하기). rig 은 `results/frame-path-overhead-2026-08-23/` 의 `overhead_rig.py` 를 쓴다(리눅스·규모·GIL 팔 포팅 [#396](https://github.com/Shadowfit/init/pull/396)). ✅ **2026-08-23 결정: 2대다** — 대상 `c7i.4xlarge` + 부하기 `c7i.xlarge`. 기준 조건 「346 RPS · 9.5 vCPU」가 **부하기 별도 박스**에서 난 관측이라 1대로 옮기면 재현 대상이 달라지고, 부하기 몫(평균 0.5 · 피크 1.4 vCPU · §T 실측)이 **설명하려는 6.5 vCPU 의 8~22%** 다([결정](../docs/decisions/r10-loadgen-topology.md)). 🔴 **배선 넷이 아직 없다**([#400](https://github.com/Shadowfit/init/issues/400)) — 러너 phase · venv ROLE · 부하기 `grpcio` · **SSH 팔 전환**. 박스 뜨기 **전에** 만들 것 |
...
- [ ] 🔴 **R10 — 프레임 경로 계측 본 판.** 現 미실행 從 항목 중 **박스 조건이 까다로운 유일한 것**이다:
      **16 vCPU**(= `c7i.4xlarge`)에서만 질문이 성립한다 — 「9.5 of 16」이 그 박스의 관측이다.
      8 vCPU 박스로도, 로컬 물리 2코어로도 **못 잰다**(후자는 R6 이 확인한 벽).
      **소요: 본 측정 ≈34분 · 인스턴스 1시간**([격자](../docs/decisions/ai-receive-path-scaling.md) §13-7) · 도커·MySQL·시딩 **불필요**(venv + `frames.json` + 부하기).
      🔴 **AI CPU 샘플러가 없으면 태우지 말 것** — 도커가 없어 `docker stats` 가 없고, 그게 없으면 「9.5 of 16」을 못 만진다(#400 다섯째).
      계측은 켜도 된다(대가 ≤2.4%, `results/frame-path-overhead-2026-08-23/`) — **ON/OFF 앵커 판 한 장**은 끼울 것.
      ✅ **2026-08-23 결정: 2대다**(대상 `c7i.4xlarge` + 부하기 `c7i.xlarge`) — 기준 조건이 그 모양이었다
      ([결정](../docs/decisions/r10-loadgen-topology.md)). 🔴 **그래서 이 항목은 «박스만 있으면 되는» 從 이 아니다** —
      배선 넷([#400](https://github.com/Shadowfit/init/issues/400): 러너 phase · venv ROLE · 부하기 `grpcio` · SSH 팔 전환)이
      **박스 뜨기 전에** 서 있어야 한다. 안 서 있으면 R6 처럼 «박스를 갖고도 못 태우는» 자리가 된다.
