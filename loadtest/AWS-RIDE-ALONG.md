...
| **R9** | **관측 스택 동거 비용 — AI 부하 기준** (Q5 · 가설 H5) | 설계의 **팔 D**(= 팔 C + `--profile obs`)를 격자에 한 팔로 얹는다. 🔴 **P6-b 전용이다** — 다른 라운드에는 **AI 무대가 없다**(P4 는 두 대 다 MySQL 이고 부하기·AI 컨테이너·`frames.json` 이 전부 없다). 08-18 세션분산 라운드가 **쓰기 경로 기준**으로 대체 관측했지만(처리량 차 **−0.05%** · 스택 CPU 1% 미만) **두 부하는 병목이 다르다** — AI 부하는 CPU 포화 조건이라 «스택이 쓰는 1%» 가 처리량에서 그대로 빠질 수 있다. **판정선은 설계 그대로**: D 가 C 대비 **두 자릿수 % 내려가면 H5 반증**. ⚠️ **從 중 유일하게 «추가 비용 거의 0» 이 아니다** — 판이 **19개(+42분)** 는다. 1·2라운드 모두 「라운드가 길어지면 뺀다」로 제외됐으므로 **P6-b 격자를 확정할 때 넣을지부터 정한다** | [`ai-coresidency-capacity.md` §3(팔 D) · §6 H5 행](../docs/decisions/ai-coresidency-capacity.md) · [재고표 Q5](../docs/decisions/experiment-inventory.md) · 소요 근거는 §11 「이번에도 안 하는 것」 |
...
| **R10-b** | **프레임 경로 계측 — 2대 판** — 「~9 에 눌리나」 | 🟡 **모양은 정해졌고(ㄱ 2대) 착수는 미정** — [`r10-loadgen-topology.md` §7](../docs/decisions/r10-loadgen-topology.md). `handler_concurrency` 분포를 **기준 조건**(부하기가 별도 박스)에서 잰다. 🔴 **이건 안 쪼개진다** — 절대값이라 부하기 몫(평균 0.5 · 피크 1.0~1.4 vCPU = 설명하려는 6.5 vCPU 의 **8~22%**)이 그대로 섞인다. **착수는 R10-a 를 보고 다시 정한다** — 거기서 GIL 노브가 처리량을 **안** 움직이면 1순위가 지워지고 이 판의 격자가 줄어든다. 배선(SSH 팔 전환 + 부하기 박스 `grpcio`·생성 스텁)도 그때 필요한 만큼만 만든다. 🔴 **두 판의 값을 한 표에 못 놓는다** — 라운드 간 비재현(+17.7%)이 사이에 낀다 | [무대 결정](../docs/decisions/r10-loadgen-topology.md) · **2대**(대상 `c7i.4xlarge` + 부하기 `c7i.xlarge`) · 배선 **미작성** · 소요 미추정 |
| **R11** ⭐ | **박스 보정값 — 이 인스턴스가 초당 얼마나 «일» 하나** | 🟢 **rig 이 이미 있다 · 라운드당 1~2분** — `profile_e2e_and_scaling.py scaling` 의 **1워커 판**이 그대로 이것이다([R6 이 쓴 것](results/ai-scaling-aws-2026-08-17/README.md), 그때 값 **79.9 fps**). 도커·MySQL·부하기 **전부 불필요**(venv + `frames.json`) | **「라운드를 건너면 절대값이 안 맞는다(+17.7%)」의 축 0** — [설계 §2·§5](../docs/decisions/round-to-round-nonreproducibility.md). `docker stats` 의 CPU% 는 **시간**이지 «일의 양» 이 아니라서, 물리 호스트의 유효 클럭이 다르면 **같은 CPU% 로 다른 양의 일**을 한다 — 관측된 서명(«AI CPU 869.3%→869.0% 로 같은데 처리량 +17.7%»)이 정확히 그 모양이다. 단일 스레드 고정 작업이라 동시성·네트워크가 빠지고 **그 박스의 작업률만** 남는다. 🔑 **매 라운드 상설로 걷는 것이 요점이다** — 한 판만으로는 비교 대상이 없다. 🔴 **과거는 못 메운다**(설계 §6: P6 1·2라운드 박스는 사라졌다). 이 항목이 답하는 것은 **「앞으로」** 다. ⚠️ **설계 전체(축 A·B — 인스턴스 K대·재부팅 M회)는 여전히 미확정**이고, 여기 올리는 것은 **EC2 결정이 필요 없는 축 0 뿐**이다([#498](https://github.com/Shadowfit/init/issues/498) ①) | [설계](../docs/decisions/round-to-round-nonreproducibility.md) · [검토 #498](https://github.com/Shadowfit/init/issues/498) · 재고표 AI 축 3번 |
| **R12** | **읽기 p99 를 판정선에 댄다** — 로컬 판이 못 하는 그 한 가지 | 🔴 **이 從 은 «넘겨졌는데 등록이 안 돼 있던» 몫이다.** 재고표 1번이 *「판정선 대면은 **EC2 從** 몫」* 으로 넘겨놓았는데 **탑승 목록에 그 행이 없었다**([재고표 §2 1번](../docs/decisions/experiment-inventory.md)) — 박스가 떠도 후보로 안 올라온다. 🟢 **rig 은 이미 있다** — [`read_p99.js`](results/http-read-p99-2026-08-23/read_p99.js) · k6 `ramping-vus` 1→4→8→16(각 20초, 총 80초) · 판당 **~1.5분**. **로컬 1판이 낸 것**: p50 순서(`daily` 152 < `calendar` 180 < `weekly` 206 < `session` 248)가 **p99 에서 무너진다** — `calendar` 가 **922ms 로 제일 나쁘고** `session` 은 878ms(p99/p50 **5.1x ↔ 3.5x**). 🔴 **그 절대값을 판정선(읽기 p99 ≤ 1s)에 대면 안 된다** — 2코어에 MySQL·백엔드·AI 가 동거하는데 **k6 까지 같은 박스**이고, [`slo-baseline.md`](../docs/decisions/slo-baseline.md) 가 스스로 «시간 임계는 이 환경에서 안 쓴다» 고 못박았다. **EC2 가 그 둘을 다 뺀다.** 🔴 **그리고 반복이 1판뿐이라 산포를 모른다** — 판정선 대면이면 «넘었다/안 넘었다» 가 산포 안인지 말할 수 있어야 한다. 최소 **버림 1 + 4판** | 🔴 **실제 비용은 rig 이 아니라 시드다.** rig 이 시드를 자동화하지 않는다([결과 §1-1](results/http-read-p99-2026-08-23/README.md)) — 시더가 `member_id` 를 **1·5·12 로 하드코딩**해서, 로그인할 수 있는 계정엔 읽을 데이터가 없다. 로컬 판은 **① 계정 생성 → ② 그 `member_id` 로 세션·리포트 1,000건 시드 → ③ 리포트 붙은 세션 id 를 `K6_SIDS` 로** 를 **손으로** 밟았다. 🔑 **박스에서 이걸 손으로 하면 從 이 아니다** — 태우기 전에 그 세 단계를 스크립트로 만들어야 한다. ⚠️ **쓰기축은 이 從 이 아니다** — `POST /exercises/sessions` 는 **AI gRPC 세션을 실제로 만들어** 「HTTP 지연」이 아니라 **「AI 동거」를 재게 된다. 조건 설계가 선행**이고 rig 도 [#510](https://github.com/Shadowfit/init/pull/510) 으로 **미머지·미실행**이다 | [결과](results/http-read-p99-2026-08-23/README.md) · [`slo-baseline.md` 11번](../docs/decisions/slo-baseline.md) · 재고표 [Spring 축 1번](../docs/decisions/experiment-inventory.md) |
| **R12** | **구멍이 `DROP PARTITION` 을 바꾸는가 — 조용한 박스에서** | 🟢 **rig 완성 · 판당 약 1분 · 인스턴스 불필요** — [`q2_partition_rig.sh`](results/row-shape-frag-2026-08-24/q2_partition_rig.sh). MySQL 하나만 있으면 된다(앱·부하기·AI 무대 전부 불필요) | **[행 모양 × 파티션 설계](../docs/decisions/row-shape-partition-interaction.md) 의 Q2** — 설계가 «본체» 라 부른 쪽이다. **보존정책(DROP PARTITION) 비용이 사용자 삭제량에 의존하는가**를 정한다. 의존한다면 「DROP 1.8초 · DELETE 대비 625배」를 구멍 유무와 무관하게 인용할 수 없다. 🔴 **로컬에서 시도했고 못 갈랐다**([결과 Q2](results/row-shape-frag-2026-08-24/README.md)) — 페이지는 팔을 정확히 가르는데(같은 50,000행에 **+69~84%**) DROP 시간의 **팔 안 산포가 7.6배**라 두 팔이 완전히 겹쳤고, **페이지 최다 판이 최속**이었다. 설계의 「상대비교까지 로컬」이 **틀렸다** — 조용한 박스가 필요하다. ⭐ **덤이 더 클 수 있다**: 한 판에서 파티션 둘을 연달아 DROP 하면 **먼저 한 쪽이 느리다**(부호가 뒤집힌다). 「DROP 1.8초」 류 인용이 **첫 DROP 이었는지 둘째였는지**에 의존하는데 그 조건이 기록돼 있지 않다 — 조용한 박스면 그것도 같이 갈린다 | [설계 §3 Q2](../docs/decisions/row-shape-partition-interaction.md) · [로컬 판](results/row-shape-frag-2026-08-24/README.md) · 재고표 MySQL 축 6번 |
...
- **2026-08-24: 從 R12(읽기 p99 판정선 대면)를 등록한다 — «넘겨졌는데 주인이 없던» 몫이다.**
  재고표 §2 1번이 *「판정선 대면은 EC2 從 몫」* 으로 넘겨놓았는데 **탑승 목록에 그 행이 없었다.**
  박스가 떠도 후보로 안 올라오는 상태였다. 🟢 rig(`read_p99.js`)·조건은 이미 서 있고 판당 ~1.5분이다.
  🔴 **다만 «추가 비용 거의 0» 이 아직 아니다** — 시더가 `member_id` 를 하드코딩해서 로컬 판이
  계정 생성·시드·세션 id 추출 **세 단계를 손으로** 밟았다. **그 자동화가 이 從 의 선결**이고,
  안 하면 從 이 아니라 主 만큼 손이 간다. ⚠️ **쓰기축은 여기 안 들어간다** — AI 동거를 재게 되어
  조건 설계가 선행이다(#510 미머지).
