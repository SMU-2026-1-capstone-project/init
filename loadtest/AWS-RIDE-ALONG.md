...
상태: **5회 탑승 완료 (P1 2026-08-12 · P3 2026-08-13 · P3-b 2026-08-13~14 · P6 2026-08-16 · P6 2라운드 2026-08-17) + 팔 B 교정 1판 (2026-08-14)**
      ...
      남은 主 는 **P4 의 Q1·Q2 뿐**이고, 그것도 설계 문서가 «EC2 2대 필요, 안 함» 으로 박제한 상태다(아래 P4 행).
...
| **P4** | **복제 지연 · 반동기 대가** | 인스턴스 2대가 전제. ⭐ **반동기 대가는 AZ 간 RTT 에 지배**되므로 로컬에선 구조적으로 과소평가된다 | 🟡 **설계 완료 (2026-08-13)** · 🔴 **「rig 없음」은 2026-08-22 에 정정됐다** — **로컬 rig 은 있다**([`results/replication-2026-08-17/`](results/replication-2026-08-17/README.md) · `replication_probe.sh` · `docker-compose.yml` · #289 · 커밋 `28fec8d`)이고 **Q3·Q4·Q5 는 그 판으로 닫혔다**. 없는 것은 **2대판**이다. ⚠️ 그리고 그건 rig 을 복사해 올리는 일이 아니다 — 로컬 판의 계측이 `docker exec` 왕복 **초 단위**라 5초 신호를 7~11.5초로 찍었고(팔 C 가 잡았다), **AZ 간 RTT 를 재려면 그보다 가는 계측이 선행**이다. 설계: [`../docs/decisions/replication-lag-and-semisync.md`](../docs/decisions/replication-lag-and-semisync.md) | 게이트 2~3h + 측정 2~3h |
...
- [ ] **P3·P4 착수 여부** — ~~설계 문서부터 써야 한다. 그 전엔 탑승 대상이 아니다~~
  ...
  🔴 **2026-08-22 정정**: 「설계만 서고」가 아니다 — **로컬 라운드가 2026-08-17 에 돌았고**(Q3·Q4·Q5),
