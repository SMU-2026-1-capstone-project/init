...
| **R12** | **HTTP 쓰기 경로 p99 — 판정선 대면** ([slo-baseline §4-2](../docs/decisions/slo-baseline.md)) | 🟢 **rig 이 이미 있다 · 라운드 ~30분** — `PHASES="httpwrite collect"`. 「세션 쓰기 p99 ≤ 300ms」에 대응하는 실측이 **0** 인 자리다. 읽기 절반은 2026-08-23 로컬 판이 채웠지만 그 판이 스스로 «부하기 동거라 판정선에 대면 안 된다» 고 적었다. 🔴 **부하기 박스에서 돌린다**(`ROLE=p6-loader` → `TARGET_HOST`) — 같은 박스면 단계가 스스로 멈춘다. 팔은 VU 가 아니라 **가정 피크 배수**(×60·×180·×360)라, 산출이 「N ms」가 아니라 **「가정 피크의 몇 배까지 300ms 를 지키나」** 가 된다 | [`measure_http_write_p99.sh`](measure_http_write_p99.sh) · [`k6/write_p99.js`](k6/write_p99.js) |
