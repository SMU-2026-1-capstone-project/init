"""§3.3 AWS 정식 라운드용 — `coresidency-2026-08-15/load_ai.py` 사본 + 계정 세팅 페이싱.

## 왜 사본인가 (원본을 안 고치는 이유)

원본은 `coresidency_sweep.sh`·`overhead_rig.py`·`sticky_rig.py` 가 같이 쓰는 검증된 rig다.
여기서 고치면 그 세 rig 의 재현성이 같이 흔들린다 — `synthetic_body_e2e.py`가 `synthetic_body.py`
원본을 안 건드리고 사본을 만들었던 것과 같은 이유로 사본을 판다.

## 무엇을 고쳤나 (2026-08-30, §3.3.7 1차 라운드 실패 원인)

`setup_account`(회원가입→로그인)를 세션 수만큼 페이싱 없이 연속 호출하면
`AuthRateLimitFilter`(IP당 60회/60초, **경로별 독립 버킷** — `/member/signup`과 `/member/login`
은 따로 센다, `AuthRateLimitFilter.java:99` `key = URI + "|" + remoteAddr`)에 막힌다. 1차
라운드는 세션 160개를 무페이싱으로 돌리다 59번째 계정에서 429를 받았다.

수정: `main()`의 계정 세팅 루프에 `--setup-pace-sec`(기본 1.5초) 간격을 넣는다. 계정 하나당
signup 1회 + login 1회가 거의 동시에 나가므로, 루프 간격 1.5초 = 60초 창에 최대 ~41회 →
한도(60) 대비 여유 ~32%. 🔴 **rate limiter 설정(`security.rate-limit.*`)은 건드리지 않는다** —
1차 라운드에서 이걸 끄려던 시도가 Claude Code 권한 분류기에 막혔고, 그게 맞는 판단이었다
(부하테스트 편의로 보안 장치를 끄는 것은 이 실험의 정당한 범위 밖).

원본 docstring(아래)은 그대로 남긴다 — 이 rig 이 무엇을 재고 안 재는지는 원본과 동일하다.
프레임만 `frames.json`(실제 스쿼트 영상, §3.3.1)으로 교체됐다는 점도 원본과 다르다.
...
def main():
    ...
    ap.add_argument("--setup-pace-sec", type=float, default=1.5, dest="setup_pace_sec",
                    help="계정 세팅(가입+로그인) 간 최소 간격. AuthRateLimitFilter "
                         "60회/60초(경로별)에 여유를 두는 값 — 기본 1.5s = 창당 최대 ~41회")
    a = ap.parse_args()
    ...
    print(f"준비 — 계정 {a.sessions}개 (prefix={a.user_prefix}, 페이싱 {a.setup_pace_sec}s/계정 "
          f"→ 예상 {a.sessions * a.setup_pace_sec:.0f}초)")
    toks = []
    for i in range(a.sessions):
        ...
        if tok is None:
            ...
            sys.exit(1)
        toks.append(tok)
        # AuthRateLimitFilter 여유 확보 — signup·login 각각 경로별 버킷(60회/60초)이라
        # 계정당 간격을 두면 두 버킷 다 한도 밑에 머문다.
        elapsed = time.monotonic() - t_setup
        if elapsed < a.setup_pace_sec:
            time.sleep(a.setup_pace_sec - elapsed)
