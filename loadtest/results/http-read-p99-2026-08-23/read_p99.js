...
// ── 쓰는 법 ──────────────────────────────────────────────────────────────────
//
// 🟢 **준비는 스크립트 하나다** (2026-08-24, 從 R12 선결):
//
//   loadtest/seed/seed_k6_read_account.sh
//
//   계정 생성 → 로그인 확인 → 그 member_id 로 세션·프레임·리포트 시드 → K6_SIDS 까지
//   찍어준다. 마지막에 나오는 k6 실행줄을 그대로 붙이면 된다.
//
// 🔴 **왜 스크립트가 필요했나** — 시더(`loadtest/seed/seed_report_rig.sh`)가 member_id 를
//    1·5·12 로 하드코딩해서, 새로 만든 계정에는 읽을 데이터가 없다. 그렇다고 시드 회원의
//    비밀번호를 갈아끼우면 안 된다 — 남의 계정 자격증명을 만지는 일이다.
//    대신 **내가 만든 계정에 데이터를 시드**한다(README §1). 2026-08-23 판은 이 셋을 손으로 밟았다.
//
