...
커밋 + 컨테이너 로그)는 **gp3 기본 처리량 상한(125MiB/s)을 실제로, 반복적으로, 지속적으로
채운다.** 원인이든 결과든 이 조합에서 gp3 기본 처리량으로는 이 부하를 못 받는다는 사실 자체는
바뀌지 않는다 — 처리량을 프로비저닝하거나(gp3는 별도 과금으로 최대 1,000MiB/s까지 살 수
있다) io2로 옮기는 것이 다음 실용적 손잡이다. 이 결정 자체는 안 한다
([[feedback_no_arbitrary_threshold_values]] — 얼마를 살지는 실측 없이 못 정한다).
