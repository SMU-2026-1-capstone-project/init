...
> 🔴 **2026-09-02 정정 — 위 단락은 §6(2차 재현) 이전 시점의 최선 추정이고, §6-3 이 그걸
> 반증했다.** MySQL은 두 사고 다 사고 직전까지 완전히 idle 했다. **지금 최선 설명은
> §6-7(2026-09-02 추가) — EBS(gp3) 볼륨이 두 사고 모두에서 기본 처리량 상한(125MiB/s)에
> 그대로 눌러붙어 있었다**(CloudWatch 사후 조회, `docker exec`·MySQL 쿼리·백엔드 응답이
> 전부 디스크에 물려 있다는 점과 들어맞는다). §5의 "미검증" 항목이 이걸로 닫혔다.
...
- ~~🆕 **EBS(gp3) IOPS/처리량 크레딧 소진** — §6이 새로 세운 후보. 미검증~~ → ✅ **§6-7에서
  CloudWatch 사후 조회로 확인됨(2026-09-02)** — 두 사고 모두 EBS 볼륨이 기본 처리량 상한
  (125MiB/s)에 정확히 눌러붙어 있었다. **"크레딧 소진"은 부정확한 표현이었다** — gp3는
  버스트 크레딧이 없고(gp2/st1/sc1 전용 개념), 있는 건 늘 켜져 있는 고정 상한이다
...
- ~~**EBS I/O 지표를 안 걷어서 §6-4의 새 가설(I/O 크레딧 소진)은 검증이 아니라 추정이다**~~ →
  ✅ **§6-7에서 CloudWatch 사후 조회로 닫힘(2026-09-02)** — 이 라운드 자체는 안 걷었지만,
  AWS가 볼륨 단위로 이미 기록해뒀던 값을 나중에 꺼내 봤다
- **§6-5의 OOM 인과관계(원인/결과)를 못 가른다**

### 6-7. EBS 처리량 상한 — CloudWatch 사후 확인 (2026-09-02 추가)

**새 EC2 라운드를 안 띄우고 답했다.** 이 두 인스턴스는 이미 terminate됐지만, EBS 볼륨의
CloudWatch 지표는 **볼륨이 사라진 뒤에도 남는다**(리전 기본 보존 15개월) — 볼륨 ID만 찾으면
됐다. `aws cloudwatch list-metrics --namespace AWS/EBS --dimensions
Name=InstanceId,Value=<인스턴스 ID>`로 1차(`i-049fc27ab2a98bc0e` → `vol-0f43c8a7ebe35b26b`)와
2차(`i-054ee168874fd51df` → `vol-09c177250eb4e5470`) 볼륨을 각각 찾아, `VolumeAvgThroughput`·
`VolumeAvgIOPS`(5분 간격, 이 계정은 기본 모니터링이라 그보다 잘게는 없다)를 사고 전후로 조회했다.

**1차(07:31:54 부하 시작 · 07:32~07:38 사이 멎음 · 08:06:38 리부트)**

| 시각(UTC) | AvgThroughput (KiB/s) | AvgIOPS |
|---|---:|---:|
| 07:20 | 412 | 21.6 |
| 07:25 | 448 | 22.5 |
| 07:30 | 128,074 | 2,237 |
...
**07:25→07:30 사이(정확히 07:31:54 부하 시작 직후) 128,075 KiB/s ≈ 125.07MiB/s로 뛰어오르고,
리부트 직전(08:05)까지 35분 내내 그 값에 그대로 눌러붙어 있다.** 128,000 KiB/s(=125MiB/s)가
gp3의 **기본 처리량 상한**이다 — 이 정도로 정확히 일치하는 숫자가 우연일 확률은 낮다. IOPS는
1,400~2,300 사이로 3,000 기본 상한 **밑**이었다 — **막힌 건 IOPS가 아니라 처리량**이다.

**2차(09:28:27 부하 시작 · 09:38:29~09:38:45 세 채널 동시 정지 · 09:43:03 리부트)**
...
**09:38:29~09:38:45(세 독립 채널이 동시에 멎은 그 순간)을 담는 5분 구간(09:35~09:40)에서 값이
54%→100%로 올라가 정확히 상한에 닿는다.** 09:45(리부트 이후)엔 다시 21,881로 떨어진다 —
**두 번 다 같은 서명**이다.

⚠️ **`VolumeIOPSExceededCheck`·`VolumeThroughputExceededCheck`·`VolumeStalledIOCheck`
(AWS의 EBS 상태 확인 지표)는 두 볼륨·이 날 하루 전체에 데이터 포인트가 하나도 없다.** 위
숫자가 이렇게 명백한데도 이 "공식 경보성" 지표들은 조용했다 — 이 계정·리전·볼륨 조합에서
이 지표들이 신뢰할 수 있는 채널이 아니거나(기본 모니터링에선 발행이 뜸한 것으로 보인다),
최소한 **이 지표만 보고는 이 사고를 절대 못 잡는다**는 뜻이다. 원 숫자(`VolumeAvgThroughput`)
쪽이 훨씬 믿을 만하다.

**여전히 안 갈리는 것 — 상관관계이지 인과관계의 증명은 아니다.** 처리량이 상한에 눌러붙은
"동안" 시스템이 얼어있었다는 것과, 처리량 상한이 "그 얼어붙음을 일으켰다"는 것은 다른
주장이다. 다만 이 EC2 인스턴스는 **루트 볼륨 하나에 OS·컨테이너·MySQL 데이터 파일이 전부
얹혀 있다** — 그 볼륨의 쓰기가 막히면 systemd 저널·docker exec·MySQL 쿼리·백엔드 응답이
"거의 동시에" 죽는 것은 자연스러운 결과다(§6-4가 본 바로 그 셋). 반대 방향(먼저 뭔가가
멎어서 앱이 재시도 폭주로 쓰기를 늘렸다)도 배제는 못 한다 — 상한에 **닿은 뒤에도** 앱이
계속 쓰기를 시도했다면 큐만 쌓이지 처리량 숫자 자체는 똑같이 상한에 붙어 있을 것이기
때문이다. 이걸 가르려면 앱 쪽 쓰기 요청량(예: MySQL `Innodb_data_writes`)을 같은 시간축에
놓고 "요청이 상한보다 먼저 늘었는지, 상한에 닿은 뒤에 밀린 것뿐인지"를 봐야 하는데, **그
지표를 이 라운드가 안 걷었다** — 이건 이 두 인스턴스가 사라진 지금은 영영 못 메운다.

**그래서 실용적으로 남는 결론**: 이 워크로드(계정 193~203개 동시 가입·로그인·온보딩 + MySQL
커밋 + 컨테이너 로그)는 **두 사고 모두에서 gp3 기본 처리량 상한(125MiB/s)에 도달했다** —
반복적이고 지속적이다.

🔴 **여기서 멈춘다.** 「그래서 gp3 기본 처리량으로는 이 부하를 못 받는다」는 **이 판이 못 하는
말이다** — 상한 도달과 사고 사이의 **인과가 미확정**이고(§6-7), 애플리케이션이 실제로 얼마를
썼는지도 **안 걷었다**. 상한에 닿는 것과 그것이 박스를 죽이는 것은 다른 주장이다.

**가설로 남긴다**: 처리량 프로비저닝(gp3는 별도 과금으로 최대 1,000MiB/s)이나 io2 전환이
다음 손잡이 «후보» 다. 채택은 **통제된 재현으로 인과를 먼저 세운 뒤**이고, 얼마를 살지는
실측 없이 못 정한다([[feedback_no_arbitrary_threshold_values]]).
