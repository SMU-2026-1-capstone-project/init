...
#
# 소요(버림판 2판 실측): 시딩 745~1,261s + DDL(A 990s / B 2,360s) → 8판 총 **약 5.9시간**.
#   밤에 돌릴 것. 초판 주석의 «3시간 내외» 는 팔 B 를 재기 전 추정이라 두 배 어긋나 있었다.
#   파일 복원 시딩(probe_import_speed.txt, 판당 851s→39s)을 채택하면 약 3.8시간.
#
# 무인 실행 전에는 축소 리허설을 먼저 돌린다 — SESSIONS=134 (10만 행, ~15분). README 참고.
