...
세션 분석 계산 시 `joint_coordinates`(2.3KB JSON, InnoDB off-page 저장) 전체를 헛로드하던 경로를 3컬럼 DTO projection으로 교체:
...
> 📌 **조건** — ① 위 표에서 보이듯 **두 작업의 행수가 같지 않다**(8,301,450 vs 7,560,000, 약 9.8% 차). `625배` 는 소요시간의 raw 비율이고 **행당 정규화하면 약 570배**다. ② 🔴 **DROP 1.8초는 진짜 O(1) 이 아니다** — ~910MB 파일 삭제 I/O 가 들어 있고 로컬 디스크가 느려서 1.8초다(빠른 스토리지면 sub-100ms). 「O(1)」은 *행단위 비용 0* 이라는 뜻이지 상수 시간이 아니다. ③ 로컬·더미 데이터 기준이며, AWS real-JSON **축소 서브셋**(52.5만 행) 재현에서는 **421배**였다(행당 DELETE 비용은 0.135 → 0.127 ms/행으로 비슷 — 메커니즘 결론은 동일). 조건 전체는 [`portfolio/realmysql-experiments.md §②d`](./portfolio/realmysql-experiments.md).
...
| 읽기 | JSON projection | payload −98.7%, warm 쿼리 8x(로컬 412만 행) → 29~41x(AWS 1억 행)<br>⚠️ precompute 잡의 자원 절감이지 사용자 체감 지연 아님 — **정상 조회는 이 쿼리를 안 탄다**(§5.2 📌), 3컬럼 시절 값 |
| 인덱스 | 풀스캔 대조 | 인덱스 부재 시 85초 |
| 페이지네이션 | offset→keyset | 최대 489,868x |
| 보존 | DROP PARTITION | DELETE 대비 625x (행당 정규화 570x, 로컬·더미 — §5.5 📌) |
