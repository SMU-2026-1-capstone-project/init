...
> ## 🔴 2026-08-08 — 이 미팅이 열렸다는 흔적이 없고, 차단 요소는 그대로다
>
> 이 문서는 *"모든 BE 작업(BE-13/14/15) + AI handoff + Front 통합의 **작업 시작 차단 요소**"* 를 30분 안건 5개로 정리했다. **2.5개월 뒤 확인 결과 그 차단이 풀리지 않았다.**
>
> | | 2026-08-08 확인 |
> |---|---|
> | AI 측 결함 분류·송신 | ❌ **미착수** — `ai-server/` 에 `FeedbackBatch` 사용 0건 ([`ai-tts-feedback-batch.md`](./ai-tts-feedback-batch.md)) |
> | Spring 수신부·proto·테이블·시드 | ✅ 전부 있다 |
> | 프론트 TTS 통합 | ❌ 확인되지 않음 |
>
> 즉 **막힌 것은 «합의» 가 아니라 «착수» 다.** 이 문서가 전제한 *"합의가 안 돼서 시작을 못 한다"* 는 진단이 지금은 맞지 않을 수 있다 — Spring 쪽은 합의 없이도 만들 수 있는 것을 다 만들었고(수신부·테이블·시드), 남은 것은 AI 의 분류 로직처럼 **합의보다 설계·구현이 먼저인 항목**이다.
>
> 📌 **그래서 이 안건표를 그대로 다시 쓰기 전에 한 번 갱신하는 게 맞다.** 5개 안건 중 몇 개는 그 사이 코드로 결정됐을 가능성이 있다(gRPC 통일·ET-H 는 2026-05-26 에 이미 박제됐다 — [`tts-negotiation-checklist.md`](./tts-negotiation-checklist.md)). **결정된 것을 다시 안건에 올리면 30분이 그것으로 다 간다.**
>
> ⚠️ **미검증**: 미팅이 실제로 열렸는지, 열렸다면 무엇이 정해졌는지는 **저장소에서 알 수 없다.** 이 블록은 *"흔적이 없다"* 까지만 말한다.

마지막 업데이트: 2026-05-25 · 상태 확인: **2026-08-08**
...
## 안건 3 — endpoint prefix 인증·토큰 분리 — **✅ 해소됨 (2026-05-26 gRPC 통일)**

> **2026-05-26 해소**: 본 안건 자체가 *AI ↔ Spring REST internal endpoint 가 공존하던 상태* 를 전제로 했으나, gRPC 통일 결정으로 internal REST endpoint 가 모두 사라짐 (`InternalFeedbackController` 삭제). 남은 REST 는 *클라 ↔ Spring* 만이며 전부 JWT (Spring Security). `application.yml` 의 `/internal/**` whitelist 도 제거됨. AI ↔ Spring 인증은 gRPC `Authorization: Bearer` (`InternalAuthInterceptor`) 로 단일화. **별도 prefix 정책 결정 불필요.**

### 결정 사항
> ☑ 무효화 — 2026-05-26 gRPC 통일 결정으로 분기 자체 소멸. 참고: [`../decisions/tts-design.md`](../decisions/tts-design.md) 상단 박스.
