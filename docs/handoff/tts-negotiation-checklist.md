...
> ## 🔄 2026-08-08 상태 확인 — 28건의 «협의» 는 진척이 없고, 그 사이 구현이 한쪽만 갔다
>
> 이 문서는 *"구현 시작 전 3자가 합의해야 할 경계 계약 28건"* 이고, 2026-05-26 에 11건이 해소됐다. **그 뒤로 이 문서에 추가된 해소 기록은 없다.**
>
> 그런데 구현 상태는 그대로가 아니다 (2026-08-08 코드 확인):
>
> | 층 | 상태 |
> |---|---|
> | proto (`ReportFeedbackBatch`) | ✅ 양쪽 |
> | Spring 수신·저장 (`FeedbackLogService`, 테이블, 시드) | ✅ |
> | **AI 분류·송신** | ❌ **0건** — `ai-server/` 에 `FeedbackBatch` 사용처 없음 |
> | 프론트 TTS 발화 | ❌ 확인되지 않음 |
>
> 🔴 **이 문서의 전제가 흔들린다.** *"협의 전엔 구현하지 않는다"* 가 전제였는데 **Spring 은 협의 없이 만들 수 있는 것을 다 만들었다.** 그래서 지금 남은 28건 중 일부는 *"합의해야 할 것"* 이 아니라 **"이미 한쪽이 정해버린 것"** 일 수 있다 — 다시 열 때는 **협의 항목과 기정사실을 먼저 갈라야 한다.** 안 그러면 협의가 이미 구현된 계약을 뒤집는 방향으로 흐른다.
>
> 📌 지금 이 기능의 병목은 협의가 아니라 **AI 측 착수**다 → [`ai-tts-feedback-batch.md`](./ai-tts-feedback-batch.md), [`../tasks/30-ai-remaining-work.md`](../tasks/30-ai-remaining-work.md) §1(1순위).
>
> ⚠️ **미검증**: 28건 각각을 코드와 대조하지는 않았다. 위 표는 **송신 경로 유무**만 확인한 것이다 — 개별 안건(발화 시점·문구·중복 억제 등)의 현재 상태는 별도 감사가 필요하다.

마지막 업데이트: 2026-05-26 (gRPC 통일 + ET-H 결정 박제) · 상태 확인: **2026-08-08**
