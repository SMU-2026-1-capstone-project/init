...
| **탭 전환 / 앱 백그라운드 / 뒤로가기** | **아무것도 안 함.** 세션은 서버에 열린 채, 폴링은 계속 돎 |
...
## 2. 탭을 바꿔도 카메라 폴링이 계속 돈다 ⚠️

`(tabs)` 네비게이터에 `unmountOnBlur` / `freezeOnBlur` 설정이 없다 (`app/(tabs)/_layout.tsx:14-31`). 그래서 다른 탭으로 이동해도 운동 화면이 **마운트된 채로 남는다.**

폴링 `useEffect` 의 의존성은 `[isRecording, sessionId, exerciseId]` (`exercise.tsx:199`) 라 탭 전환으로는 재실행되지 않는다. 결과적으로 **330ms 인터벌이 그대로 살아서** `takePictureAsync` 와 `POST /api/v1/pose` 를 계속 호출한다 (`exercise.tsx:194`).

즉 사용자가 다른 탭을 보고 있는 동안에도 **초당 3회 카메라 촬영 + 서버 요청**이 나간다. 배터리·데이터·서버 부하가 전부 영향을 받고, 화면에 보이지도 않는 프레임이 `pose_data` 에 쌓인다.
