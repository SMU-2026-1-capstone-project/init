...
>
> 🔴 **공식에 «딱 맞추지 말 것.** 이 공식은 **검출기만** 센다 — 프레임 버퍼·base64 임시·
> 파이썬 힙·gRPC 는 **미측정이라 안 들어갔다**(`memory_ceiling()` 이 스스로 다는 경고).
> 딱 맞추면 **풀이 세션을 거절하기 전에 컨테이너가 OOM 으로 죽고**, 그 순간 위의 «실패 방향은
> 비대칭» 이 뒤집힌다 — «덜 받아짐»(안전)이 아니라 «죽음»(위험)이 된다.
> ⚠️ 여유를 «몇 %» 로 할지는 **여기 적지 않는다.** 그 값이 미측정이라 숫자를 박으면 그게
> 출처 없는 기준값이다([[feedback_no_arbitrary_threshold_values]]). 재는 것은 #229.
