...
| `AI_MEM_LIMIT` | AI 컨테이너 메모리 한도 | 🔴 **운영 필수.** prod compose 는 값이 없으면 **`up` 이 멈춘다.** compose 를 안 거치면 «첫 세션에서 죽는다» (#214) — 아래 |
...
> **배포 직후가 아니라 첫 사용자에게** 나타난다.
>
> ⚠️ **어디서 실패하는지가 배포 방식에 따라 다르다.**
>
> | 배포 방식 | `AI_MEM_LIMIT` 없을 때 |
> |---|---|
> | `docker-compose.prod.yml` | ✅ **`up` 이 멈춘다** — `${AI_MEM_LIMIT:?…}` 가 컨테이너 생성 전에 거른다 |
> | 직접 `docker run` · k8s 등 | 🔴 **`up`·헬스체크 통과 후 첫 세션에서 죽는다.** 위 경로가 없다 |
>
> **값 유도(x86 EC2 실측 적합, #229·2026-09-02)**: 한도 = 708MiB + 106.74MiB × 동시 세션 수
> (`docs/decisions/detector-pool-ceiling-formula.md`). 프레임 버퍼·base64 임시·파이썬 힙·gRPC
> 몫이 이제 포함돼 있다 — 옛 공식(100.5+98.7×N, 검출기만 계산)보다 상한이 낮게 나온다.
> 실패 방향은 비대칭이다 — 작게 주면 «세션이 덜 받아짐»(돌긴 한다), 크게 주면 «안 뜸».
>
> 🔴 **그래도 공식에 «딱 맞추지 말 것.»** 이 값은 **안전계수가 아니라 실측 적합**이고, 측정된
> 기계·워크로드(x86 c7i.4xlarge) 밖은 검증되지 않았다. 딱 맞췄는데 다른 조건이면 **풀이
> 세션을 거절하기 전에 컨테이너가 OOM 으로 죽을 수 있고**, 그 순간 위의 «실패 방향은 비대칭»
> 이 뒤집힌다 — «덜 받아짐»(안전)이 아니라 «죽음»(위험)이 된다.
> ⚠️ 여유를 «몇 %» 로 할지는 **여기 적지 않는다.** 근거 없이 숫자를 박으면 그게
> 출처 없는 기준값이다([[feedback_no_arbitrary_threshold_values]]).
