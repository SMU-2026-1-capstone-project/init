...
- **멱등 수신**: `SessionService.applyComplete`의 `if (status==COMPLETED) return` (first-write-wins) — 중복 콜백 안전.
...
- **읽기 축**: 인덱스(부재 시 85초 대조), **keyset 페이지네이션**(offset 대비 최대 489,868x), JSON **projection**(payload −98.7%, warm 쿼리 8x→**29~41x** — 412만 행 로컬 → 1억 행 AWS 재검증에서 강화, 단 precompute 잡의 자원 절감이지 사용자 체감 지연 아님), **버퍼풀**(작업셋 vs 풀)
...
- 🟢 **자기 처방이 자기 근거를 무효화한 것을 잡아냈다** (2026-08-08). 커넥션 풀 병목을 세 번 다른 곳에서 찾았다 — 로컬 2코어 박스 → (분리 배포 실측) 커넥션 풀, cliff 10~15 → **(격자 재측정) 풀이 아니다, 절벽 자체가 없음**. 마지막 반전의 원인이 **우리가 넣은 다운샘플(R=5)** 이다: 배치당 25행 → 5행이 되면서 풀이 병목에서 빠졌고, 그래서 `pool=15` 를 정당화한 실험이 낡았다. `DOWNSAMPLE_WINDOW` 를 되돌린 **대조군으로 인과를 직접 확인**했다(RPS 1.7배·p99 4.9배).
  ...
  >
  > 📌 **여기서 방법론 오류를 하나 더 기록했다 — 내 실험 설계가 답을 낼 수 없는 모양이었다.** 첫 스윕은 *"배치를 키우면 +32%"* 를 냈는데, 순서를 뒤집자 *"작게 하면 +36%"* 가 나왔다. 팔마다 1판씩이면 «팔» 과 «판 순서» 가 같은 축에 겹쳐 **원리적으로 분리가 안 된다.** 버림판 + 라틴 방격 3라운드로 다시 재서 부호를 확정했다.
  >
