...
>
> ⚠️ **그래서 이 행의 갭도 성격이 바뀐다.** "커넥션풀 한도"는 **초당 ~205건 수준에서는** 레버가 아니다(pool 5~20 동급). 🔴 **그런데 폭주가 아플 자리가 어디인지는 이 실험이 못 짚었다** — 풀도 CPU 도 포화가 아닌데 처리량이 ~205 에 고정이고, 1순위 용의자는 **부하기 커넥션 1개**(ghz 14판 전부 `connections: 1`, 미검증)다. 백프레셔·rate limit 이 없다는 사실 자체는 그대로 유효하다.
