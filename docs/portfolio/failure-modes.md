...
> - **Blast 🔴→🟡**: c=100 부하에서 **풀이 포화돼도(pool=5 구간에서 `pending` 95) `timeout_total` 은 0 이었다** — 커넥션당 작업이 짧아(5행 INSERT) 30초 한도에 안 걸린다. **포화 ≠ 붕괴.**
>   🔴 **단 이 행의 근거는 약하다** (2026-08-08 재검증). 초판에 적었던 *"`pending` 96, 대기 7.28초"* 는 **다른 구간(R=25 대조군·v1)의 값**을 인용한 것이었다. 게다가 스크레이프 간격이 **15초**인데 판 하나가 **약 10초**라, `c100-pool20` 판은 창 안에 **샘플이 0개**다. 즉 **게이지를 판별로 귀속시킬 수 없다** — "포화였다"는 방향만 신뢰하고 수치는 인용하지 않는 게 맞다. 경위: [결과 README §5](../../loadtest/results/pool-cliff-2026-08-08/README.md)
> - **완화 🔶→🟡**: 다운샘플 R=5 가 배치당 25행 → 5행으로 줄이면서 **풀이 병목에서 빠졌다** — `pool=20` 은 20개 중 2~3개만 쓰고(대기 0) `pool=5` 는 포화인데(95 대기) 처리량이 같다.
>   🔴 **어디로 옮겨갔는지는 모른다** (2026-08-08 재검증). 초판은 *"백엔드 CPU 로(백엔드 p90 128% vs MySQL 53%)"* 라고 적었는데 **두 수치 다 커밋된 원본에 없다** — MySQL 지표는 아예 수집되지 않았고(`metrics/` 10종 전부 백엔드), 스윕 중 백엔드 `system_cpu` 는 **0.51~0.56** 으로 포화가 아니었다. **철회한다.**
>
> ⚠️ **그래서 이 행의 갭도 성격이 바뀐다.** "커넥션풀 한도"는 **초당 ~205건 수준에서는** 레버가 아니다(pool 5~20 동급). ✅ **폭주가 아플 자리는 같은 날 저녁에 특정됐다 — DB 의 커밋 `fsync`** ([`../../loadtest/results/ceiling-fsync-2026-08-08/`](../../loadtest/results/ceiling-fsync-2026-08-08/)). 풀도 CPU 도 부하기도 아니었다(`--connections` 1→16 무변화, 세 박스 CPU 유휴). 내구성 설정만 바꿔 231.6 → 803.1 RPS.
>
> **그래서 이 행의 실패 모드가 분명해진다**: 폭주하면 **처리량은 ~230 에서 안 늘고 지연만 선형으로 길어진다**(c/RPS=지연이 정확히 성립). 죽지는 않는다(timeout 0) — **느려질 뿐이다.** 백프레셔·rate limit 이 없다는 사실은 그대로 유효하고, **없으면 큐만 길어진다**는 게 이제 실측으로 뒷받침된다.
> 🔴 **버퍼풀 사이징(§④)의 근거가 사라졌다는 판정도 되돌린다** — *"MySQL 이 놀고 있었다"* 는 **수집한 적 없는 지표로 내린 결론**이었다.
