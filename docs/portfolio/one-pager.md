...
| 인덱스 유무 대조 | 약 **9,000배** | 2026-07-20 로컬 재측정, 412.5만 행, **실제 2.1KB JSON**, `SET profiling`(wall-clock 오염 배제). 인덱스 ~3.0ms ↔ `IGNORE INDEX` 풀스캔 ~27.1초. 더미 JSON이면 **660배**로 과소평가된다(행이 커지면 풀스캔이 밟는 페이지가 늘어 배수가 증폭). ⚠️ 과거의 «7,000배» 는 다른 실험 수치를 짜깁기한 추정이라 **폐기된 값**이다 | [§②① · 조건](./realmysql-experiments.md#index-9000x) · rig `loadtest/measure_admin_index.sh` |
...
| **인덱스 설계·검증** | `EXPLAIN`으로 이미 최적임을 확인 후 **가설 폐기**, `IGNORE INDEX` 강제 풀스캔과 직접 대조 | 약 9,000배 (조건: ①표) | [§②① · 조건](./realmysql-experiments.md#index-9000x) · rig [`measure_admin_index.sh`](../../loadtest/measure_admin_index.sh) |
