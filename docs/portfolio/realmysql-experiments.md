...
- **결과 (d) 파티션 ✅ (2026-06-03, 1억 행 → `created_at` 월별 14파티션, in-place ALTER)**:
  ...
  - **⭐ DROP PARTITION vs DELETE WHERE (둘 다 ~8M 행대 만료 — 단 행수가 정확히 같지는 않다, 아래 조건표)**:
  ...
  - 📌 **이 수를 인용할 때 같이 가야 하는 조건 (2026-08-17 박제)** — 값은 정확하다. 빠져 있던 것은 **조건**이다. 인용처 9곳에 이 표를 단서로 달았다(«−98.7%» 12곳·«3.47배» 13곳과 같은 작업).

    | 축 | 조건 |
    |---|---|
    | **행수** | 🔴 **두 작업의 행수가 같지 않다** — DELETE 8,301,450행 vs DROP 7,560,000행(약 9.8% 차). `625배` 는 **소요시간의 raw 비율**(18.6분 ÷ 1.8초)이고, 행당으로 정규화하면 **약 570배**다. 둘 다 맞지만 인용할 땐 어느 쪽인지 밝힐 것 |
    | **O(1)** | 🔴 **DROP 1.8초는 진짜 O(1) 이 아니다.** ~910MB `.ibd` 파일 삭제 I/O 가 들어 있고, 로컬 디스크가 느려서 1.8초다 — **빠른 스토리지면 sub-100ms**. 「O(1)」이 뜻하는 것은 *행단위 비용이 0* 이라는 것이지 상수 시간이 아니다. 따라서 **배수는 스토리지에 따라 더 커질 수도 있다** |
    | **환경·재현** | 원본은 **로컬·더미 데이터**(830만 행). AWS 재검증은 전체 1억 행이 아니라 **축소 서브셋**(2파티션, old 52.5만 행/~1.2GB real JSON)이고 그 배수는 **421배**다. 배수가 작아진 것은 스케일 축소 영향이고, **행당 DELETE 비용은 오히려 비슷**(0.135 → 0.127 ms/행)해서 메커니즘 쪽 결론은 그대로다 |
    | **적용 범위** | 파티셔닝의 정당화는 **TTL 하나뿐**이다 — 세션 단위 조회의 **pruning 이득은 0**(별도 실측으로 반증, 위 «pruning» 항목). 「1억 행이니까 파티션」이 아니다 |
