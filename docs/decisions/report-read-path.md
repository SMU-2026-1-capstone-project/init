...
| # | 문제 | 상태 | 코드 근거 |
|---|------|------|-----------|
| ① | ~~**fat 컬럼 over-fetch** — 풀엔티티 로드, worst 계산은 `syncRate`·`timestampSec`·`feedbackMessage` 3개만 씀. `joint_coordinates`는 **한 번도 안 씀**~~ | **✅ 해결(2026-07-15)** | `findFramesBySessionId`(3컬럼 projection, §7)로 교체. 실측(§②b): payload −98.7%, warm 쿼리 **8x**(로컬 412만 행) → **29~41x**(AWS 1억 행, 2026-07-15 재검증). ⚠️ 조건은 §7 아래 📌 — **정상 조회는 이 쿼리를 안 탄다**(precompute·폴백 전용) |
...
| projection 효과 | 작음(행 작음) | **큼(−98.7%)** — 단 precompute·폴백 경로 한정(§7 아래 📌) |
...
| 등급 | 항목 | 의미 |
|---|---|---|
| 🟢 **실재 + 측정** | ① over-fetch (projection −98.7% 측정) | 코드에 있고 합성 볼륨으로 측정. 단 **그 쿼리가 사는 경로는 precompute·폴백**(§7 아래 📌) |
