...
>
> **양쪽 다 ~205 RPS 로 같았다는 사실 자체가 오히려 신호다.** 부하기를 옮겼는데도 천장이 안 움직였다면, 천장을 만든 것은 **박스 공유가 아닌 다른 것**이다. 🔴 **1순위 용의자가 부하기 자신이다** — ghz 결과 14판이 전부 `"connections": 1` 이다. 즉 **동시성 c 를 100 으로 줘도 TCP 커넥션 하나에 다중화**했을 수 있고, 그러면 측정한 천장은 서버가 아니라 **부하기 설정**의 것이다. **미검증**이며, 다음 실험의 1번 확인 항목이다.
>
