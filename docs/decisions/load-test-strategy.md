...
- **백엔드 부하 관점**: 거의 아무것도 아님.
  ...
  # 🔴 2026-08-23 에 역산으로 복원한 감춰진 가정 둘 — 이 줄에 원래 없었다:
  #   세션 길이 15분(application.yml:252 «스쿼트 예상시간», 타임아웃 계산용 예시값이지 실측 아님)
  #   피크 1시간 집중도 18%(repo 어디에도 없던 값)
  #   1,000 × 1.5 × 0.18 × (15/60) = 67.5   ← 이래야 67 이 나온다
  # 둘 중 하나만 움직여도 DAU 1,000 안에서 천장(89세션)에 닿는다:
  #   세션 30분 → DAU 659 · 피크 25% → DAU 949   (ai-receive-path-scaling.md §3-ㄹ-1)
