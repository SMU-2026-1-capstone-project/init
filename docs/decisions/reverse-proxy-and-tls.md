...
### 8-2. 도메인 없이 지금 할 수 있는 준비

프록시를 어느 쪽으로 고르든 **백엔드가 할 일은 인증서가 아니다**(§7). 그리고 그 일은 지금
바로 할 수 있다:

- `application-prod.yml` 이 **아예 없다**(§1-4). `spring.profiles.active: prod` 가 기본인데
  그 프로파일 파일이 없어서 dev 와 prod 가 같은 설정으로 돈다
- 프록시를 넣으면 `server.forward-headers-strategy`(X-Forwarded-* 신뢰)가 들어갈 자리가
  바로 거기다. 그 파일이 없으면 **넣을 자리부터 만들어야 한다**
- 이걸 미리 해두면 ④가 정해졌을 때 남는 일이 «프록시 설정» 하나로 줄어든다
