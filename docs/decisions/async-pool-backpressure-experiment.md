...
- 워밍업: Caffeine 3종(`exercises` · `exerciseReferences` · `feedbackTemplates`, `expireAfterWrite=1h`)이
  cold 면 시작 경로가 느려 첫 판이 오염된다
- **투입 규모 캘리브레이션**: «큐 길이가 관측 가능해지는 최소 투입» 을 여기서 찾는다.
  🔴 **미리 숫자를 박지 않는다** — 근거 없는 임계값이 하나 더 생기는 것을 막는다([[feedback_no_arbitrary_threshold_values]])
- 버림판 결과는 **버린다.** 규모 파라미터만 다음 단계로 넘긴다
...
| 결과 | 다음 |
|---|---|
| 서킷이 큐 성장을 실질적으로 막는다 | ✅ **«서킷브레이커가 백프레셔 역할까지 한다» 를 실측으로 확보.** 회고 ⑥(클라 백프레셔 없음)에 대한 **서버 쪽 답**이 된다 — 클라엔 없지만 서버엔 있다 |
| 서킷 OPEN 뒤에도 큐가 자란다 | 🔴 결함. 이슈 등록. 완화는 `queue-capacity` 상한 + 거절 정책인데, **거절의 «모양» 은 P3(입장 제한)와 같은 설계 문제**라 거기서 합류시킨다 |
| 서킷이 설정과 다른 지점에서 연다 | 🔴 결함. 다섯 숫자 중 어느 것이 안 먹는지 좁혀서 이슈 |
| 큐가 애초에 안 자란다 | H1 이 틀린 것. **그 자체가 산출물** — «기본값을 그대로 뒀다» 에 근거가 붙고, 더 볼 게 없다는 판정으로 닫힌다 |

**네 갈래 중 셋이 이슈 또는 실측 자산이고, 나머지 하나는 닫힘이다.** 빈손으로 끝나는 갈래가 없다.

---

## 8. 미결정 (사용자 confirm 필요)

- [ ] **착수 여부** — §6 이 «잉여» 라고 적었다. 그럼에도 열 것인지
- [x] ~~**0단계만 먼저 할지**~~ — **0-a 완료(2026-08-11).** 앱을 띄울 필요도 없었다. 0-b(런타임 대조)는 스택이 뜰 때 곁다리로
- [ ] **OS 샘플러 확장을 여기에 얹을지** — four-axes §4 가 «DDL 스윕 재실행 전에 붙이는 게 가장 싸다» 고 했다. 이 실험이 먼저 오면 순서가 바뀐다
- [ ] **결과 위치** — `loadtest/results/async-pool-<날짜>/` 관례를 따를지
- [ ] **샘플러 폴링 간격** — §4-2. 근거를 어떻게 세울지(간격 자체를 예비 판으로 정할지)
