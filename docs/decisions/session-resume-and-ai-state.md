...
| B | **재부착 엔드포인트** | 세션 이어감. rep은 `rep_number` 추가 시 복원 | 저~중 | **⭐ 추천(A와 함께)** |
| C | AI 상태 영속화(Redis 등) | 거의 완전 | 중 | 2순위. 별도 카드 |
...
- AI 쪽은 변경이 거의 없다 — `SessionStateRegistry.create:69-78`이 `self._sessions[session_id] = state`라 **같은 id로 다시 부르면 자연히 덮어쓴다.**
