...
## 7. 미검증 → **2026-08-17 게이트 라운드에서 둘 다 해소**

[결과](../../loadtest/results/payload-uniqueness-gate-aws-2026-08-17/README.md) · `c7i.xlarge` · 커밋 `629e7f0`

- ~~**sprig `mod` 가 int64 `RequestNumber` 를 받는지**~~ ✅ **받는다.** 3,000요청 판에서
  `rep_number` 가 0~2999 로 요청마다 증가하고 세션 100개에 고르게 흩어졌다.
  **저장 행 15,000 = 요청 3,000 × 5, 실패 0** — 중복이 하나도 안 삼켜졌다
- ~~**템플릿 실행 비용의 실제 크기**~~ ✅ **쟀다.** 같은 동시성 대조에서 **요청당 부하기 CPU 가
  2.2배**(1.20ms → 2.63ms). 환산하면 649 RPS 에서 **약 1.7 vCPU** —
  🔴 **부하기는 4 vCPU 여야 한다. 2 vCPU 면 85% 로 부하기가 먼저 포화한다**
- 🔴 **대신 새 미검증이 열렸다** — 이 라운드는 **1대 박스**라 부하기·대상이 CPU 를 공유했다.
  전용 부하기에서의 절대값은 재확인이 필요하다(방향은 믿을 만하다)

### 7-1. 🔴 §0 의 서술 하나를 정정한다

*"`fail=0` 에 RPS 도 정상으로 찍히므로 표를 봐서는 안 보인다"* 는 **동시성에 따라 갈린다**:

| 동시성 | 옛 페이로드의 겉모습 |
|---|---|
| **c=1** | 300요청 전부 OK · 행은 500뿐 — **서술대로 완전히 조용하다** |
| **c=10** | **34.8%가 `[Internal]`** — 중복이 삼켜지는 게 아니라 **데드락**이 난다 |

P5 는 c=100 이라 **조용한 오측정이 아니라 데드락 폭풍**을 봤을 것이다. 판을 못 쓰는 결론은
같지만 원인을 **잠금 쪽으로 잘못 쫓았을** 것이다. 그리고 그 데드락은 rig 만의 문제가 아니다 →
[#276](https://github.com/Shadowfit/init/issues/276)
