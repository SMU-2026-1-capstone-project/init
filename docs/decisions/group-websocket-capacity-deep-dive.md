...
작성일: 2026-08-30 (§12 REST+WS 결합부하·풀 크기 통제 실험 추가 2026-09-02)
상태: **§1-1~§1-5(단일 그룹 동시성) + 다중 그룹 스파이크(§9, AWS) + §10~§11(HikariCP 경계 재검증, AWS) 실행 완료.** 단일 인스턴스 범위는 사실상 종결. §12가 REST(세션 시작)까지 얹은 축을 로컬에서 통제 실험(풀 크기 15→30)으로 열었다 — §10~§11(핸드셰이크·발행 단독)과 다른 결론이 나왔고, 완전히 닫히진 않았다(§12-4).
...
연관: [`sse-capacity-deep-dive.md`](./sse-capacity-deep-dive.md)(같은 방법론을 트레이너 SSE에 적용한 짝 문서, 이 문서가 그 구조를 그대로 따른다), [`four-axes-depth-experiments.md`](./four-axes-depth-experiments.md), [`group-full-user-journey-scenario.md`](./group-full-user-journey-scenario.md)(§12가 여는 REST+WS 결합 축의 본체), [[feedback_tps_over_dau_justification]], [[project_loadtest_env_constraint]], [[feedback_measure_design_needs_repeats]]
...
- **HikariCP 풀 대기가 처음으로 관측됐다** — §1-5(로컬, 최대 M=40)까지는 `hikaricp_connections_pending`이 항상 0이었는데, 이번 300연결에서 처음으로 **15**가 잡혔다. 커넥션 풀(`maximum-pool-size: 15`)이 병목이 되는 규모가 로컬 실측 범위(≤40) 밖, 이 AWS 실측 범위(300) 안 어딘가에 있다는 뜻 — 정확한 경계는 이 두 실측 사이에 있다는 것만 확정. **🔴 정정(§11-4): 이 15는 반복으로 재현 안 되는 단발 관측이었다 — 인용 금지, §11 참고.**
...
- **새로 열린 질문(§10~§11·§12에서 다뤄짐)**: (a) HikariCP 풀 병목의 정확한 경계 — §10·§11이 핸드셰이크·발행 단독 경로에서 반복 판으로 재검증해 "이 15는 단발 노이즈, 그 경로엔 애초에 경계가 없다"로 정정했다(§11-4). §12가 이 축에 REST(세션 시작)를 더해 로컬에서 다시 열었고, 거기서는 실제로 재현 가능한 압력 신호가 나왔다 — 즉 §9의 "경계"는 틀린 질문이었지만, REST를 얹은 §12의 경계는 별개로 실재한다(§12 참고). (b) 비정상 종료 감지 지연의 정확한 크기 — **§9-5에서 재측정 후 정정됨, 아래 참고**.
- **인프라**: `c7i.xlarge` 1대, 총 가동 ~20분, 종료·보안그룹 룰 회수까지 완료.

### 9-5. 정정 (2026-08-31) — "3초와 60~120초 사이 어딘가"는 측정 오염이었다

`group-websocket-heartbeat.md` §6에서 heartbeat 도입 여부를 검토하며 위 §9-3의 감지 지연을 정밀 재측정했다. §9-3은 범용 `jvm_threads_live`를 3초 간격으로만 보고 나머지는 "1~2분 뒤 수동 확인"이라 부정확했다 — 이번엔 `GroupSocketRegistry`에 전용 게이지(`shadowfit_group_ws_active_sessions`)를 새로 노출해 0.5초 간격으로 정확한 드롭 시점을 쟀다.

**결과 (2026-08-31, 로컬)**:

| 세션 수 | 판 | 연결 성공 | 감지까지 |
|---|---|---|---|
| 3 | 5판 | 3/3 × 5 | 0.2~1.1s (중앙값 0.4s) |
| 150(§9-3과 동일 규모) | 3판 | 150/150 × 3 | 0.8s, 0.9s, 1.6s |

**§9-3의 "60~120초"는 틀렸다.** 같은 기법(WS Close 프레임 없이 raw `sock.close()`)·같은 규모(150연결)로 재현했는데도 감지는 전부 2초 미만이었다 — 자릿수 자체가 다르다. 원인은 §9-3이 쓴 지표(`jvm_threads_live`, 다른 스레드 변동과 섞이는 범용 지표)와 성긴 폴링(3초 간격 뒤 "1~2분 후 수동 확인")의 조합으로 추정된다 — 이번엔 레지스트리 전용 게이지로 오염 없이 직접 쟀다.

**파급**: `group-websocket-heartbeat.md`가 이 "60~120초"를 heartbeat 도입의 핵심 근거로 삼았었다 — §9-5의 정정이 그 문서의 §1 결론을 뒤집는다. 자세한 내용과 후속 결정은 그 문서 §1-수정·§7 참고.

---

## 10. §9-4(a) 후속 — HikariCP 경계를 k6로 좁히려다, 전제 자체가 깨졌다 (2026-08-31, AWS)

**목적**: §9가 "40(로컬, pending=0)~300(AWS, pending=15) 사이 어딘가"로 열어둔 경계를 k6(`loadtest/k6/group_ws_handshake_concurrency.js`)로 이등분 탐색해 좁힌다.

**환경**: 로컬은 이번에도 못 썼다 — 다른 세션의 V11 마이그레이션 파일이 실시간으로 생겼다 사라지길 반복해 백엔드가 Flyway 검증 크래시 루프에 빠졌고, 별도로 뜬 세션의 `heartbeat-test` 스택이 CPU 218%를 먹어 로컬 박스 자체가 포화 상태였다([[project_concurrent_sessions]]). 워크트리(`feat/group-realtime-websocket-loadtest`, 커밋 `f41fab00`)로 격리해도 도커 빌드가 5분 넘게 네트워크조차 못 만들 만큼 막혀 있어, AWS로 넘어갔다(`c7i.xlarge` 1대, ROLE=db 부트스트랩 후 backend만 수동 빌드·기동 — §9와 같은 경량 프로비저닝).

### 10-1. 이등분 탐색 — 300까지 pending이 계속 0

| M | 판 수 | hikari_pending_max | handshake p50 범위 | 실패 |
|---|---|---|---|---|
| 10 | 1 | 0 | 52ms | 0 |
| 150 | 3 | 0 (전부) | 85~151ms | 0 |
| 225 | 3 | 0 (전부) | 79~115ms | 0 |
| 260 | 3 | 0 (전부) | 62~180ms | 0 |
| 280 | 3 | 0 (전부) | 69~103ms(1판 max 1137ms 스파이크) | 0 |
| **300** | 3 | **0 (전부)** | 144~180.5ms | 0 |

🔴 **300에서 §9의 pending=15가 재현되지 않았다.** §9와 정확히 같은 연결 수(300)인데 이번엔 한 번도 안 걸렸다.

### 10-2. k6의 "동시성"이 느슨한가 의심 — 동기화 배리어를 넣고 재시도

k6 스크립트 자체 주석이 이미 "VU 초기화가 순차적이라 python의 `threading.Barrier`만큼 동시가 아니다"를 경고하고 있었다. `setup()`에서 공유 목표 시각(`startAt = Date.now() + max(3000, M*10)ms`)을 계산해 모든 VU가 `sleep()`으로 그 시각까지 대기했다가 동시에 `ws.connect()`하도록 스크립트를 고쳤다(측정 도구 수정 — 서비스 코드 아님).

| M | 조건 | 판 수 | hikari_pending_max |
|---|---|---|---|
| 300 | 동기화 후 | 3 | **0 (전부)** |
| **450** | 동기화 후 | 2 | **0 (전부)** |

**450(§9의 300 대비 1.5배)까지도 안 걸렸다.** 동기화 배리어를 넣어도 결과가 안 바뀌었다 — VU 시작 타이밍 문제가 원인이 아니었다는 뜻이다.

### 10-3. 재해석 — 애초에 같은 것을 재고 있지 않았다

§9(`measure_group_ws_multi_group_spike.py`)의 300연결 판은 **접속 후 각자 이벤트를 1건씩 발행**했다(§9-2가 "접속만으로는 스레드 비용조차 실측에 안 걸린다"며 스크립트를 고친 바로 그 이유). 이번 k6 판은 **핸드셰이크만 하고 즉시 소켓을 닫는다** — 이벤트 발행이 없다.

핸드셰이크(`JwtHandshakeInterceptor`)는 인덱스 커버된 SELECT 두 번뿐이라 커넥션 점유 시간이 짧다(§1-5가 이미 이렇게 확정: HikariCP 획득 최대 21ms). 반면 이벤트 발행(`GroupEventService.publish()`)은 비관적 쓰기 잠금 + INSERT + 커밋이고, §1-3이 이미 InnoDB 카운터로 "동시 발행자 M명 중 M-1명이 행 잠금 대기"를 확정해뒀다 — 락 대기 중에는 커넥션을 계속 붙들고 있다.

즉 **§9의 pending=15는 핸드셰이크가 아니라 "핸드셰이크 직후 동시 발행"이 만든 압력이었을 가능성이 높다.** §9는 이 둘을 분리하지 않고 한 번에 쟀고, 이번 라운드가 핸드셰이크만 떼어내 보니 그 압력이 사라졌다 — **두 실측이 애초에 같은 축을 재고 있지 않았다.**

> 🔴 이건 추측이다 — 확정하려면 §9와 똑같이 "접속+발행"을 k6로 재구성해 pending이 다시 뜨는지 봐야 한다(이번 라운드에서는 안 함, 시간·과금 배분 문제로 여기서 멈췄다). 확정된 것은 **"핸드셰이크만으로는 450까지도 HikariCP 풀이 안 눌린다"**는 사실이다.

### 10-4. 결론

- **핸드셰이크 단독 경로**: HikariCP 풀(15)은 450 동시 핸드셰이크까지 무죄로 확정(동기화 배리어 유무 무관, 실패 0건 유지).
- **§9의 40~300 "경계"라는 질문 자체가 재구성 대상이다** — 원래 전제(같은 것을 다른 규모로 쟀다)가 틀렸을 가능성이 높다. 진짜 압력원 후보는 발행 경로의 락 대기 시간(§1-3)이지 핸드셰이크가 아니다.
- **개선 옵션은 전부 보류** — HikariCP `maximum-pool-size` 상향·`JwtHandshakeInterceptor` 쿼리 병합·멤버십 캐싱, 이 셋 다 핸드셰이크 경로를 겨냥한 옵션인데 이번 라운드가 그 경로에서 병목을 못 찾았다. 적용할 근거가 없다 — **미확정, 코드 변경 없음.**
- **인프라**: `c7i.xlarge` 1대, ROLE=db 부트스트랩 후 backend 수동 빌드(AI·nginx·관측 스택 스킵), 총 가동 약 1.5시간. terminate 확인 완료.

---

## 11. §10의 가설 검증 — "발행 경로가 압력원"도 재현 안 됨, 진짜 천장은 Tomcat 스레드풀 (2026-08-31, AWS)

**가설**: §10-3이 추측으로 남긴 것 — §9의 pending=15는 핸드셰이크가 아니라 "핸드셰이크 직후 즉시 발행"이 만든 압력이었을 것이다. 이번 라운드가 §9와 똑같은 결합부하(접속 → 각자 자기 그룹에 이벤트 1건 발행)를 **반복 포함해서** 재현한다.

**방법**: `measure_group_ws_multi_group_spike.py`(§9가 쓴 그 스크립트)에 `--seed-out`/`--seed-in`을 추가해(측정 도구 수정) 계정·그룹 준비와 스파이크 실행을 분리 — 매 판 계정을 새로 만들면(레이트리밋 페이싱 1.25s/계정) 반복 비용이 감당 안 돼서다. `c7i.xlarge` 신규 인스턴스(§10과 별도 라운드, ROLE=db + backend 수동 빌드).

### 11-1. 이등분 탐색 — 결합부하로도 300까지 pending은 계속 0

| 그룹×멤버 | 총 연결 | 판 수 | hikari_pending_max | tomcat_busy_max | 실패 |
|---|---|---|---|---|---|
| 67×3 | 201 | 3 | 0 (전부) | 80~145 | 0 |
| 83×3 | 249 | 3 | 0 (전부) | 135~159 | 0 |
| 92×3 | 276 | 3 | 0 (전부) | 56~165 | 0 |
| **100×3** | **300** | 3 | **0 (전부)** | 73~152 | 0 |

🔴 **§9와 정확히 같은 구성(100그룹×3명, 접속+발행)을 3판 반복해도 pending=15가 재현되지 않았다.** §9는 이 조건을 **1판만** 돌렸다([[feedback_measure_design_needs_repeats]]가 경고해온 바로 그 함정) — 지금 보면 그 1판의 15는 진짜 임계값이 아니라 **폴러(50ms 간격) 샘플링이 우연히 잡은 순간 스파이크**였을 가능성이 높다.

### 11-2. 450까지 밀어보니 — HikariCP가 아니라 Tomcat 스레드풀이 먼저 걸린다

| 그룹×멤버 | 총 연결 | 판 | hikari_pending_max | tomcat_busy_max | handshake p50 |
|---|---|---|---|---|---|
| **150×3** | **450** | 1 | 0 | **200 (cap)** | 385ms |
| 150×3 | 450 | 2 | 0 | 177 | 428ms |
| 150×3 | 450 | 3 | 0 | 79 | 319ms |

**450에서 처음으로 `tomcat_threads_busy_threads`가 Tomcat 커넥터의 상한(200)에 정확히 닿았다** — 그런데도 **실패는 0건**(450/450 성공, 지연만 p50 300~430ms로 늘어남). Tomcat이 풀이 꽉 차면 요청을 거절하는 게 아니라 `acceptCount` 큐에 쌓아두고 순서대로 처리한다는 뜻이다.

### 11-3. 재해석 2 — 애초에 이 경로에 HikariCP 천장은 없다

§1-1(2026-08-30)이 이미 "WS 메시지 처리가 HTTP 커넥터 스레드풀을 REST와 공유한다"를 JMX로 확정해뒀는데, 이번 라운드가 그 공유 천장에 **핸드셰이크+발행 결합 부하로 처음 도달**했다 — §1-1은 M=8~11의 작은 버스트만 봤고 풀 크기(200)에 한참 못 미쳤었다.

즉 이 경로의 실제 캐파시티 순서는:
1. **Tomcat 커넥터 스레드풀(200)** — 450 근처에서 처음 압박(§1-1이 예견한 지점, 이번에 처음 실측으로 닿음)
2. **HikariCP 풀(15)** — §10·§11 전 구간(10~450, 핸드셰이크 단독·결합부하 둘 다)에서 **한 번도 안 눌림**
3. §1-3의 seq 락 경합은 여전히 유효하지만(InnoDB 카운터로 확정된 사실), 그 대기가 커넥션을 오래 붙드는 정도가 **HikariCP 15개 풀을 고갈시킬 만큼은 아니었다** — 세션당 오는 순서가 흩어져 있어(그룹이 100~150개로 나뉨) 한 그룹 안의 동시 경합자 수(M-1) 자체가 크지 않았기 때문일 가능성이 높다(§1-3은 그룹 하나 안에서 M=2/4/8만 봤다).

### 11-4. 결론

- **§9의 "300에서 pending=15"는 반복으로 재현이 안 되는 단발 관측이었다** — 정본에서 인용 금지. §9 본문에 이 정정을 남겨야 한다(§9 자체는 안 고치고 이 절에서 정정한다).
- **HikariCP `maximum-pool-size`(15)는 이 문서가 실측한 전 범위(핸드셰이크 10~450, 결합부하 201~450)에서 단 한 번도 압박받지 않았다** — 상향할 근거가 없다. 관련 개선 옵션(풀 크기 상향·`JwtHandshakeInterceptor` 쿼리 병합·멤버십 캐싱) **전부 기각**(코드 변경 없음, 애초에 고칠 병목이 확인 안 됨).
- **실제로 근거를 얻은 것은 하나**: Tomcat HTTP 커넥터 스레드풀(`server.tomcat.threads.max`, 기본 200)이 450 동시 결합부하에서 처음 100% 포화를 실측으로 확인했다. 단 **실패로 이어지진 않는다**(큐잉만 발생) — "고쳐야 할 결함"이 아니라 §1-1이 예견한 공유 지점에 처음 닿은 관측이다. 이 축을 더 팔지(그룹 수를 더 늘려 실제 실패 지점을 찾을지)는 **미확정, 별도 결정 사안**.
- **인프라**: `c7i.xlarge` 1대(§10과 별개 인스턴스), ROLE=db + backend 수동 빌드, 총 가동 약 1시간, terminate 확인 완료.

> 🔴 **범위 한정 — §10·§11이 "무죄"로 확정한 것은 핸드셰이크·발행(그룹 이벤트) 경로뿐이다.** `POST /exercises/sessions`(세션 시작) 같은 다른 REST 엔드포인트를 같은 버스트에 얹었을 때도 HikariCP가 무관한지는 이 두 절이 답한 적이 없다 — §12가 정확히 그 축을 연다.

---

## 12. REST(세션 시작) 결합 + HikariCP 풀 크기 통제 실험 (2026-09-02, 로컬)

**배경**: [`group-full-user-journey-scenario.md`](./group-full-user-journey-scenario.md) §8이 §9-4가 미해결로 열어둔 "REST를 WS 옆에 얹으면"이라는 새 축을 실행했다 — `POST /exercises/sessions`(세션 시작)까지 버스트에 포함시킨 첫 실측이다. §10·§11(핸드셰이크·발행만, HikariCP 전 구간 무죄)과 달리 §8은 pool=15에서 **HikariCP pending이 5판 전부 0보다 컸고(9~195), 실패율 49.8%**로 나왔다 — 겉보기엔 §11-4("15는 상향할 근거 없음")와 반대 방향의 신호다.

**이 모순을 그냥 두지 않고 통제 실험으로 갈랐다** — `docker-compose.yml`에 재빌드 없이 값을 바꾸는 측정용 손잡이(`SPRING_DATASOURCE_HIKARI_MAXIMUM_POOL_SIZE: ${HIKARI_POOL_SIZE:-15}`)를 추가하고, **다른 조건은 전부 그대로 둔 채 풀 크기만 15→30으로 올려 같은 스크립트(`measure_group_full_user_journey.py`, G=100×M=3, 5판)를 재실행**했다. 격리 스택(`docker-compose.iso.yml`, `shadowfit-iso-*`)에서 실행 — 공유 dev 스택은 안 건드림.

### 12-1. 결과 — 풀을 두 배로 올리자 실패율이 절반 넘게 줄었다

| 지표 | pool=15 (`run-9f95d44e`, 2026-09-01) | pool=30 (`run-161258be`, 2026-09-02) |
|---|---|---|
| 실패 합계 | 747/1500 (49.8%) | **325/1500 (21.7%)** |
| tomcat_busy 상한(200) 도달 판 수 | 2/5판 | **5/5판 (전부)** |
| hikari_pending 판별 피크 | 31, 195, 165, 9, 177 | 108, 11, 114, 156, 176 |

- **실패율이 거의 절반으로 줄었다** — 이 실험에서 바뀐 변수는 Hikari 풀 크기 하나뿐이라, §1-3(seq 락 경합)·§1-3의 방법론과 같은 "한 레버만 바꾸는" 통제가 성립한다. Hikari 풀 크기가 이 REST+WS 결합 경로의 실패율에 **인과적으로** 기여한다는 근거다.
- **그런데 Tomcat 풀은 pool=30에서 오히려 항상(5/5판) 상한(200)에 닿았다** — pool=15일 땐 2/5판만 닿았던 것과 대조적. Hikari 압력이 풀리자 요청들이 더 빨리 다음 단계로 넘어가면서 Tomcat 커넥터 앞에 더 많이 몰린 것으로 보인다 — §11-3이 정리한 캐파시티 순서(Tomcat → Hikari)가 이 결합 경로에서도 방향은 같다: **Hikari를 완화하면 Tomcat이 그대로 노출된다.**
- **hikari_pending 평균 크기 자체는 pool=15와 pool=30 사이에 극적으로 안 줄었다**(둘 다 대략 100대 초반 평균) — 그런데도 실패율은 크게 줄었다. 즉 풀을 두 배로 늘린 게 "압력을 없앤" 게 아니라 "같은 압력 아래서 타임아웃까지 안 가고 버틸 여유를 넓힌" 쪽에 가깝다.

### 12-2. §10·§11과의 재해석 — 모순이 아니라 새 축이었다

§10·§11이 "무죄"로 확정한 것은 **핸드셰이크·그룹 이벤트 발행 경로**뿐이었다(§11 말미 범위 한정 참고). §12가 새로 얹은 `POST /exercises/sessions`(세션 시작)는 그 두 경로보다 무거운 쓰기다 — `Session`/`exercise_sessions` 행 생성과 관련 비즈니스 로직을 거친다(단, AI로의 `StartAnalysis` 통보는 커밋 후 비동기라 이 REST 응답 자체가 AI를 기다리진 않는다 — `SessionService.java` 주석 확인). §12가 보여준 것은 **"그룹 WS 자체엔 Hikari 문제가 없다"는 §11-4의 결론은 그대로 유효하지만, 세션 시작 REST를 같은 순간에 얹으면 그 경로가 Hikari를 실제로 누른다**는, §10·§11이 답한 적 없는 별개의 사실이다. 두 결론은 충돌하지 않는다 — 서로 다른 엔드포인트 조합을 쟀을 뿐이다.

### 12-3. 판별 이상치 — rep1(pool=15)·rep2(pool=30) 둘 다 설명이 안 된다

두 실행 모두 5판 중 한 판이 나머지와 동떨어진 최악의 결과를 냈다:
- pool=15의 rep1: 성공 9/300(3.0%) — 그런데 hikari_pending 피크(31)·tomcat_busy 피크(65)는 오히려 다른 판들보다 여유 있었다. 탈락은 대부분(226/291) **핸드셰이크 단계**에서 났다(§8-4가 콜드스타트 가설로 미확인 남김).
- pool=30의 rep2: 성공 55/300(18.3%) — hikari_pending 피크(11)가 이 실행 5판 중 **가장 낮은데도** 성공률은 가장 낮았다.

두 이상치 모두 "풀 압력이 클수록 실패가 는다"는 §12-1의 방향성과 안 맞는다 — 즉 이 로컬 단일 실행들은 풀 크기 효과 신호에 **설명 안 되는 큰 잡음이 얹혀 있다.** 이 박스가 물리 코어 2개에 다른 프로젝트 컨테이너들까지 동거하는 환경([[project_loadtest_env_constraint]])이라는 걸 감안하면, §10·§11 팀이 AWS 격리 + 반복 판 + 이등분 탐색까지 가야 했던 이유와 같은 종류의 잡음일 가능성이 높다.

### 12-4. 결론 — 절반만 닫혔다, §10·§11 수준의 확정은 아니다

- ✅ **방향성은 통제 실험으로 확인됨**: 다른 조건 고정, Hikari 풀만 15→30으로 올렸을 때 이 REST+WS 결합 경로의 실패율이 49.8%→21.7%로 줄었다 — 우연이 아니라 재현 가능한 레버로 보인다(단 §12-3의 잡음 때문에 "정확히 얼마나"는 말할 수 없다).
- 🔴 **§10·§11 수준(반복 판·이등분 탐색·AWS 격리)의 확정은 아니다.** 이 실험은 pool=15·pool=30 각각 **단 1회**(5판 묶음 1세트)뿐이다 — [[feedback_measure_design_needs_repeats]]가 요구하는 반복이 없고, §11-4가 §9의 1판짜리 pending=15를 "단발 노이즈"로 뒤집은 바로 그 함정을 이 실험도 그대로 안고 있다. rep1·rep2 이상치가 그 증거다.
- 🔴 **Tomcat 풀(200)이 pool=30에서 더 자주 걸린다는 것도 실패 0건으로 안 이어진다** — §11-2(핸드셰이크+발행만)는 Tomcat이 상한에 닿아도 큐잉만 되고 실패 0건이었는데, 이 REST+WS 결합 경로는 Tomcat이 상한에 닿는 판에서도 여전히 상당수 실패가 남는다(예: pool=30 rep1은 tomcat=200인데도 실패 31건). 클라이언트 측 스테이지별 타임아웃(WS접속 5s·세션시작 10s)이 Tomcat 큐잉 지연을 못 버티는 것인지, 다른 원인인지는 이 실험에서 안 갈랐다.
- **`maximum-pool-size`를 15→30으로 영구히 올릴지는 별도 결정 사안이다** — 지금 15는 다른 워크로드(대량 INSERT 배치 처리량, `commit-count-and-mysql-metrics.md` 4차 실측) 기준으로 확정된 값이라([[feedback_no_arbitrary_threshold_values]]), 이 축의 결과만으로 바꾸면 그 실측의 맥락을 지운다. 이 문서는 "풀 크기가 이 경로에 인과적으로 영향을 준다"는 방향성까지만 제공한다.
- **다음 단계(추천, 미확정)**: §10·§11과 같은 수준으로 닫으려면 이 REST+WS 결합 경로를 AWS 격리 인스턴스에서 pool=15/30(또는 추가 값) 각각 반복 판(≥3세트)으로 재실행해야 한다. 지금 남은 것은 "절반만 닫힌 인과 신호"이지 "확정된 경계"가 아니다.
