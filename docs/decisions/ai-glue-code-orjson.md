...
작성일: 2026-09-02
상태: 🟢 **N=1·N=3 둘 다 완료.** N=1(박스 61% 사용, 여유 있음)에서는 rps +7.86%(226.2 → 244.0),
p50 −6.88% — 16판 전부 방향 일관·안 겹침. 🔴 **그런데 N=3(박스 95% 포화, 실배포 조건)에서는
rps +1.62%로 줄고 통계적으로도 산포 안(우연과 구분 안 됨)** — §5 참고. **"8% 빨라진다"는 N=1
한정 문장이고, 실배포 체감은 1~2% 수준이 정직하다.**
...
**결론**: 프로세스 1개(여유 있는 조건)에서는 크게(+7.86%), 실배포 N=3(포화 조건)에서는
미미하게(+1.62%, 통계적으로도 불확실) 돕는다. "orjson으로 8% 빨라진다"는 N=1 한정 문장이고,
실배포 체감은 통계적으로 확인된 것만 말하면 **CPU 효율 쪽으로 아주 작게(+1.17%)** 남는다.

---

## 6. 이 판이 확정한 것과 안 한 것

- ✅ **글루 코드 경량화가 처리량을 올린다 — 단, 박스에 여유가 있을 때만.** N=1(박스 61%)에서
  +7.86%, 재현됨(2라운드). N=3(박스 95%)에서는 +1.62%로 줄고 통계적으로 불확실
- ✅ **판정 로직 무변경** — 면적 2파일, pytest 160판 통과
- ✅ **9.5~9.8 vCPU 천장 자체가 움직였는지 확인함** — CpuSampler로 재보니, N=1에서도 AI CPU
  사용량 자체보다 rps가 먼저 판단 근거였다(이 문서엔 N=1의 CPU 절대값을 아직 안 실었다 —
  §4는 rps·p50만 걷은 라운드였고, CPU는 N=3 라운드에서부터 걷었다). N=3에서는 "CPU를 아낀다"는
  주장이 작지만 통계적으로 확실했다(+0.44%, Welch t=5.01)
- 🔴 **더 큰 글루 코드 경량화(Pydantic 검증 스킵, 배치 등)는 안 건드렸다** — orjson은 그중
  가장 싸고 안전한 한 수였다
- 🔴 **N=3 rps 차이(+1.62%)는 판 수(n=4)가 적어 통계적으로 불확실하다** — 더 확신하려면
  판을 늘리거나(N=1처럼 8판씩) 다른 부하 수준에서 재현을 봐야 한다
...
- N=3 rps 차이를 8판씩으로 늘려 통계적으로 확실히 하기(지금은 4판씩, Welch t=1.67로 불확실)
