...
>
> 🔴 **단 «그럼 병목이 어디냐» 는 답이 없다** (같은 날 재검증으로 철회). 초판은 *"백엔드 CPU 로 옮겨갔다"* 고 적었는데 그 근거(백엔드 128% / MySQL 53%)가 **커밋된 원본에 없었다** — MySQL 지표는 수집조차 안 됐다. ~205 RPS 천장의 정체는 **미상**이고, 1순위 용의자는 **부하기 커넥션 1개**(ghz 14판 전부 `connections: 1`, 미검증)다.
>
